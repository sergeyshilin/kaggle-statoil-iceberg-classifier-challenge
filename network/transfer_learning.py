...
def predict_with_tta(model, features_model, X_data, M_data, verbose=0):
    predictions = np.zeros((tta_steps, len(X_data)))
    test_features = model.predict([X_data, M_data], batch_size=batch_size, verbose=verbose)
    predictions[0] = features_model.predict_proba(test_features)[:, 1]
    ...
    for i in range(1, tta_steps):
        ...
        predictions[i] = features_model.predict_proba(test_features)[:, 1]
...
for j, (train_index, cv_index) in enumerate(skf.split(X_train, y_train)):
    ...
    logreg = SVC(kernel='linear', probability=True, random_state=random_seed)
    train_and_evaluate_model(model, [xtr, mtr], ytr, [xcv, mcv], ycv)
    model.load_weights(filepath=best_weights_path)

    features_model = Model(inputs = model.input, outputs=model.get_layer('features_layer').output)

    train_features = features_model.predict([xtr, mtr], batch_size=batch_size, verbose=0)
    logreg.fit(train_features, ytr)

    # Measure train and validation quality
    print ('\nValidating accuracy on training data ...')
    tr_preds.extend(predict_with_tta(features_model, logreg, xtr, mtr))
    cv_preds[cv_index] = predict_with_tta(features_model, logreg, xcv, mcv)

    print ('\nPredicting test data with augmentation ...')
    fold_predictions = predict_with_tta(features_model, logreg, X_test, M_test, verbose=1)

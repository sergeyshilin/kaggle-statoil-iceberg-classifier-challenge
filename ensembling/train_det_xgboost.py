...
for xgb in range(n_xgboosts):
    features_idxs = np.arange(features_train.shape[1]) #np.random.permutation(np.arange(features_train.shape[1]))[:int(features_train.shape[1] * 0.7)]
    ...
    answer_test.to_csv('det_' + str(xgb) + '.csv', index = False)

    answer_train = pd.DataFrame({'preds': preds_train, 'is_iceberg' : dfs_cv[0].is_iceberg.values})
    answer_train.to_csv('det_cv_' + str(xgb) + '.csv', index = False)
